...
def test_get_tasks_with_empty_value_succeed(mocker):
    ...
    mocker.patch("doru.session.SessionWithSocket.get", side_effect=get_response)
    d = create_daemon_manager()
    result = d.get_tasks()
    assert result == []
...
def test_add_task_with_valid_response_succeed(exchange, interval, amount, pair, mocker):
    ...
    mocker.patch("doru.session.SessionWithSocket.post", side_effect=get_response)
    d = create_daemon_manager()
